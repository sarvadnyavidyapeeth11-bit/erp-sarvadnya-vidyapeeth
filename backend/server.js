...
const requireSupabaseAdmin = () => {
  if (!supabaseUrl || !serviceRoleKey) {
    const error = new Error("Supabase service role is not configured on the backend.");
    error.status = 500;
    throw error;
  }
};

const getRequester = async (accessToken) => {
...
const requireAdmissionRole = async (req) => {
  ...
  if (!profile || profile.status !== "active" || !["admin", "admission"].includes(profile.role)) {
    const error = new Error("Only Admin or Admission users can create student login credentials.");
    error.status = 403;
    throw error;
  }
};

const requireAdminRole = async (req) => {
  const token = String(req.headers.authorization || "").replace(/^Bearer\s+/i, "").trim();
  if (!token) {
    const error = new Error("Missing Supabase session token.");
    error.status = 401;
  ...
  if (!profile || profile.status !== "active" || profile.role !== "admin") {
    const error = new Error("Only Admin users can manage staff credentials.");
    error.status = 403;
    throw error;
  }
};
...
const createOrUpdateStudentUser = async ({ scholarNo, studentName, email }) => {
...
const createOrUpdateStaffUser = async ({ name, email, role, password }) => {
...
const updateStaffPassword = async ({ email, password }) => {
  const cleanEmail = String(email || "").trim().toLowerCase();
  const cleanPassword = String(password || "").trim();
...
const deactivateStaffUser = async ({ email }) => {
  const cleanEmail = String(email || "").trim().toLowerCase();
  if (!cleanEmail) {
...
app.get("/api/health", (_req, res) => {
  res.json({ ok: true, service: "sarvadnya-erp-backend" });
});

app.post("/api/auth/student", async (req, res) => {
  try {
    requireSupabaseAdmin();
    await requireAdmissionRole(req);
    const result = await createOrUpdateStudentUser(req.body || {});
...
app.post("/api/auth/staff", async (req, res) => {
  try {
    ...
    const result = await createOrUpdateStaffUser(req.body || {});
    res.json({ success: true, account: result });
  } catch (error) {
    res.status(error.status || 500).json({
      success: false,
      message: error.message || "Unable to create staff auth user.",
    });
  }
});

app.patch("/api/auth/staff/password", async (req, res) => {
  try {
    requireSupabaseAdmin();
    await requireAdminRole(req);
    const result = await updateStaffPassword(req.body || {});
    res.json({ success: true, ...result });
  } catch (error) {
    res.status(error.status || 500).json({
      success: false,
...
app.delete("/api/auth/staff", async (req, res) => {
  try {
    requireSupabaseAdmin();
    await requireAdminRole(req);
    const result = await deactivateStaffUser(req.body || {});
    res.json({ success: true, ...result });
  } catch (error) {
    res.status(error.status || 500).json({
      success: false,
      message: error.message || "Unable to delete staff auth user.",
    });
  }
});

app.listen(port, () => {
  console.log(`Sarvadnya ERP backend listening on http://localhost:${port}`);
});
