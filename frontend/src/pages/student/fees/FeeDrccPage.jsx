...
import {
  ...
  getFeeDetails,
  studentProfile,
  submitStudentDrccApplication
} from "../../../hooks/studentPortalData";
import { openDocumentInNewTab } from "../../../utils/openDocumentInNewTab";

const collegeDocs = [
  { name: "Bonafide / Study Certificate", desc: "Official DRCC-format bonafide study certificate" },
  { name: "Course + Duration Certificate", desc: "Approved curriculum & statutory course duration proof" },
  { name: "College Fee Structure", desc: "Authenticated year-wise approved institutional fee schedule" },
  { name: "Fee Certificate / Demand Letter", desc: "DRCC disbursement demand letter on college letterhead" },
  { name: "Enrollment / Admission Certificate", desc: "Official student admission & enrollment confirmation letter" }
];

const bsccRequiredDocs = [
  "BSCC Application Form PDF / Acknowledgement Copy",
  "Sanction Letter / Student Credit Card Approval",
  "Disbursement Proof / Bank UTR Advice to College",
  "Student & Co-Applicant Aadhaar / Identity Proof",
  "Student Bank Passbook / Cancelled Cheque",
...
const makeCollegeDoc = (name) => ({
  ...
  fileUrl: `data:text/html;charset=utf-8,${encodeURIComponent(`
    <html><head><title>${name}</title><style>body{font-family:Arial,sans-serif;padding:40px;line-height:1.6;color:#0f172a}h1{font-size:22px;color:#1e3a8a}.box{border:2px solid #cbd5e1;padding:24px;border-radius:12px;margin-top:20px;background:#f8fafc}.row{margin-bottom:10px;font-size:14px}b{color:#1e293b}</style></head>
    <body>
      ...
      <div class="box">
        ...
        <div class="row"><b>BSCC Loan / Reg ID:</b> ${studentProfile.drccLoanId || studentProfile.studentCreditCardId || "Saved on file"}</div>
        <hr style="border:none;border-top:1px dashed #cbd5e1;margin:18px 0;" />
        <p style="margin:0;font-size:13px;color:#64748b;">This college-issued document is generated for official Bihar Student Credit Card (BSCC/DRCC) loan processing and fee adjustment.</p>
      </div>
    </body></html>`)}`
});
...
export default function FeeDrccPage() {
  ...
  const reload = () => setApplications(getDrccApplications());
  ...
  useEffect(() => {
    setPaymentForm((prev) => ({
      ...prev,
      applicationNo: profileDrccId,
      loanId: profileDrccId,
      bankName: prev.bankName || studentProfile.bankName || "",
      accountNo: prev.accountNo || studentProfile.accountNo || "",
      ifscCode: prev.ifscCode || studentProfile.ifscCode || ""
    }));
  }, [profileDrccId]);
  ...
  const mine = useMemo(() => applications.filter((app) => (
    ...
    identifiers.includes(String(app.rollNumber || app.scholarNo || app.enrollmentNo || app.studentId || ""))
  )), [applications]);

  const latest = mine[0] || null;
  const bsccStatus = latest?.status || "Not Started";

  const updatePayment = (key, value) => setPaymentForm((prev) => ({ ...prev, [key]: value }));

  const submitPayment = (event) => {
    event.preventDefault();
    const documents = paymentForm.paymentProof ? [paymentForm.paymentProof] : [];
    if (!profileDrccId) {
      window.alert("Pehle My Profile me DRCC Loan / Student Credit Card ID save karein.");
      return;
    }
    const numAmount = Number(paymentForm.amountReceived) || 0;
    if (numAmount <= 0 || !paymentForm.paymentProof?.fileUrl) {
      window.alert("Amount aur disbursement credit proof screenshot valid hona chahiye.");
      return;
    }

    const submitted = submitStudentDrccApplication({
      stage: "disbursement",
      ...paymentForm,
      applicationNo: profileDrccId,
      loanId: profileDrccId,
      ...
      disbursementTo: "College Account",
      documents,
      officerRemarks: "BSCC college-account payment submitted for fee adjustment."
    });

    if (!submitted) {
      window.alert("Payment/disbursement details valid hona chahiye.");
      return;
    }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }} className="space-y-6">
      <FeeNavigationHeader
        title="Bihar Student Credit Card (BSCC / DRCC)"
        description="Download authenticated college documents for DRCC verification and submit college-account loan disbursement details for fee ledger adjustment."
        badge="BSCC / DRCC Cell"
      />
      ...
      {submittedAlert && (
        <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-900 text-xs font-bold flex items-center justify-between gap-3 shadow-xs">
          <span className="flex items-center gap-2">
            ...
            BSCC disbursement credit proof successfully submitted. The Accounts Officer will verify UTR and adjust the fee balance.
          </span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
        <div className="p-5 rounded-2xl bg-white border border-slate-200/90 shadow-xs flex items-center gap-4">
          ...
          <div className="min-w-0">
            <p className="text-[11px] font-extrabold text-slate-500 uppercase tracking-wider">BSCC Status</p>
            <p className="text-lg font-black text-slate-900 truncate mt-0.5">{bsccStatus}</p>
          </div>
        </div>
        ...
        <button
          type="button"
          onClick={() => document.getElementById("drcc-docs")?.scrollIntoView({ behavior: "smooth" })}
        ...
        <button
          type="button"
          onClick={() => document.getElementById("drcc-form")?.scrollIntoView({ behavior: "smooth" })}
        ...
        >
          ...
          <div>
            <p className="text-[11px] font-extrabold text-emerald-700 uppercase tracking-wider">Loan Fee Adjustment</p>
      ...
      <div id="drcc-docs" className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 bg-white rounded-2xl border border-slate-200/90 shadow-xs p-4 sm:p-6 space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            ...
            <span className="text-[11px] font-extrabold text-blue-700 bg-blue-50 px-2.5 py-0.5 rounded-full border border-blue-200">
              DRCC Verified Format
            </span>
          </div>
          <p className="text-xs text-slate-600 font-semibold leading-relaxed">
            These documents are auto-generated with your academic details and ready to be submitted at your district DRCC center or 7nishchay-yuvaupmission portal.
          </p>
        ...
        <div className="lg:col-span-5 bg-white rounded-2xl border border-slate-200/90 shadow-xs p-4 sm:p-6 space-y-4 flex flex-col justify-between">
          <div className="space-y-4">
            ...
            <p className="text-xs text-slate-600 font-semibold leading-relaxed">
              Required checklist for DRCC physical verification and semester loan disbursement release:
            </p>
            <div className="space-y-2">
              {bsccRequiredDocs.map((name) => (
                <div
                  key={name}
                  className="p-2.5 rounded-xl bg-slate-50 border border-slate-200 flex items-start gap-2.5 text-xs font-bold text-slate-800"
                >
          ...
          <div className="p-3.5 rounded-xl bg-amber-50 border border-amber-200/80 text-[11px] text-amber-900 font-semibold flex items-start gap-2 mt-4">
            ...
            <span>Ensure the Bank UTR / NEFT reference explicitly mentions the college beneficiary account for automatic fee clearance.</span>
          </div>
        </div>
      </div>

      {/* Main Submission Form & Records Section */}
      <div id="drcc-form" className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Left Column: Form */}
          ...
          <form onSubmit={submitPayment} className="space-y-4 text-xs">
            <div>
              <label className="block font-extrabold text-slate-800 mb-1.5">
                DRCC Loan / Student Credit Card ID (From Profile) <span className="text-rose-500">*</span>
              </label>
              <input
                required
                readOnly
                value={profileDrccId}
                placeholder="Saved in My Profile (e.g. BSCC-2025-XXXXX)"
              ...
              {!profileDrccId && (
                <p className="mt-1 text-[11px] font-bold text-rose-600">
                  Please update your BSCC Loan / Registration ID in My Profile first.
                </p>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              <div>
                <label className="block font-extrabold text-slate-800 mb-1.5">
                  Amount Disbursed / Released (Rs.) <span className="text-rose-500">*</span>
                </label>
                <input
                  required
                  type="number"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              <div>
                <label className="block font-extrabold text-slate-800 mb-1.5">
                  Bank UTR / Transaction Ref <span className="text-rose-500">*</span>
                </label>
                <input
                  required
                  value={paymentForm.utrNumber}
                  onChange={(e) => updatePayment("utrNumber", e.target.value)}
                  placeholder="e.g. UTR-BSCC-99210"
                  className="w-full px-3.5 py-2.5 bg-slate-50 hover:bg-slate-50/80 focus:bg-white border border-slate-300 focus:border-purple-600 focus:ring-2 focus:ring-purple-500/20 rounded-xl font-bold font-mono text-slate-900 text-sm transition-all"
                />
              </div>
              ...
              <div>
                <label className="block font-extrabold text-slate-800 mb-1.5">
                  Disbursing Bank / Source
                </label>
                <input
                  value={paymentForm.bankName}
                  onChange={(e) => updatePayment("bankName", e.target.value)}
                  placeholder="e.g. Bihar State Education Finance Corp"
                  className="w-full px-3.5 py-2.5 bg-slate-50 hover:bg-slate-50/80 focus:bg-white border border-slate-300 focus:border-purple-600 focus:ring-2 focus:ring-purple-500/20 rounded-xl font-bold text-slate-900 text-sm transition-all"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <label className="flex items-center justify-between gap-3 w-full px-4 py-3 bg-slate-50 hover:bg-slate-100/80 border border-dashed border-slate-300 rounded-xl cursor-pointer transition-all">
                ...
                <input
                  ...
                  onChange={(e) => readFile(e.target.files?.[0], "BSCC College Account Credit Message Screenshot", (doc) => updatePayment("paymentProof", doc))}
                  className="hidden"
                />
              </label>
            ...
            >
              <Send className="w-4 h-4" /> Submit BSCC Disbursement Proof
            </button>
          </form>
        </div>
          ...
          <div className="space-y-3.5 max-h-[580px] overflow-y-auto pr-1">
            {mine.length === 0 ? (
              <div className="py-12 px-4 rounded-2xl bg-slate-50 border border-dashed border-slate-200 text-center space-y-2">
                ...
                <p className="text-sm font-bold text-slate-700">No BSCC records submitted yet</p>
                <p className="text-xs text-slate-500 max-w-sm mx-auto font-medium">
                  When you submit your credit card loan disbursement proofs, fee ledger adjustment status will appear here.
                </p>
              </div>
            ) : (
              mine.map((app) => {
                const isApproved = String(app.status || "").includes("Completed") || String(app.status || "").includes("Adjusted");
                const isRejected = String(app.status || "").includes("Correction") || String(app.status || "").includes("Rejected");
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2">
                        <span className="font-mono font-black text-slate-900 text-sm">{app.applicationNo || app.loanId || app.id}</span>
                      </div>
                      <span
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 p-3 rounded-xl bg-slate-50 border border-slate-200 text-xs">
                      ...
                      <div>
                        ...
                        <p className="font-mono font-black text-slate-900">
                          Rs.{(app.amountReceived || 0).toLocaleString("en-IN")}
                        </p>
                      </div>
                      <div>
